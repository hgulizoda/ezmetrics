...
import { UZBCustomsFilter } from './Filter';
import { IUserId } from '../../../../types/UserId';
import { useUZBCustomsTableFilter } from './useFilter';
import { useDelivered } from '../../hook/useDelivered';
...
export const UZBCustomsPackageTable = () => {
  ...
  const { isPending: isDownloading, download } = useExcelDownload({
    name: t('packages.status.uzbCustoms'),
    params: { ...filter, status: 'to_uzb_customs' },
  });

  const addTruckDialog = useBoolean();
  const openBackDialog = useBoolean();
  const archiveDialog = useBoolean();
  const openDeliveredTruck = useBoolean();
  ...
  const delivered = (id: string, userID: string) => {
    setPackageID({ id, userId: userID });
    openDeliveredTruck.onTrue();
  };

  const sendTruckToDelivered = async () => {
    await onDelivered({ id: packageID.id, userID: packageID.userId });
    setPackageID({ id: '', userId: '' });
    openDeliveredTruck.onFalse();
  };
  ...
  return (
    <Box height={700}>
      ...
      <ConfirmDialog
        ...
        content={t('packages.actions.backTransitZone')}
      ...
      />
      <ConfirmDialog
        open={openDeliveredTruck.value}
        onClose={openDeliveredTruck.onFalse}
        title={t('packages.actions.sendDelivered')}
        content={t('packages.actions.sendDeliveredWithoutNormalize')}
        action={
          <>
            <Button onClick={openDeliveredTruck.onFalse} variant="contained" color="error">
              {t('actions.not')}
            </Button>
            <LoadingButton
              loading={isDelevered}
              onClick={sendTruckToDelivered}
              variant="contained"
              color="primary"
            >
              {t('actions.yes')}
