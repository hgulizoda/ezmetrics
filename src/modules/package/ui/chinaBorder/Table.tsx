...
import { ChinaBorderFilter } from './Filter';
import { IUserId } from '../../../../types/UserId';
import { useChinaBorderTableFilter } from './useFilter';
...
export const ChinaBorderTable = () => {
  ...
  const { isPending: isDownloading, download } = useExcelDownload({
    name: t('packages.status.chinaBorder'),
    params: { ...filter, status: 'to_china_border' },
  });
  ...
  >('chinaBorderColumns', initialColumns);
  ...
  return (
    <Box height={700}>
      ...
      <ConfirmDialog
        ...
        action={
          <>
            <Button variant="contained" color="error" onClick={archiveDialog.onFalse}>
              {t('actions.not')}
            </Button>
            <LoadingButton
              onClick={onArchive}
      ...
      <ConfirmDialog
        ...
        content={
          <Trans t={t} i18nKey="packages.actions.backChinaWarehouseContent">
