  ...
  filter() {
    ...
    for (const [key, value] of Object.entries(query_obj)) {
      ...
      if (typeof value === 'string' && Types.ObjectId.isValid(value)) {
        parsed_value = new Types.ObjectId(value)
      } else if (key.endsWith('date') || key.endsWith('_at')) {
        if (typeof value === 'string' || typeof value === 'number') {
          parsed_value = new Date(value as string | number)
        }
      ...
      } else if (typeof parsed_value === 'string') {
        if (!isNaN(Number(parsed_value))) {
          parsed_value = Number(parsed_value)
        } else if (
          parsed_value.toLowerCase() === 'true' ||
          parsed_value.toLowerCase() === 'false'
        ) {
          parsed_value = parsed_value.toLowerCase() === 'true'
        }
      }
    ...
    if (this.query_string.search && this.query_string.search_fields) {
      const search_regex = new RegExp(this.query_string.search, 'i')
      const search_fields = this.query_string.search_fields.split(',')
      match_stage['$or'] = search_fields.map((field) => ({
        [field.trim()]: { $regex: search_regex },
      }))
    }
