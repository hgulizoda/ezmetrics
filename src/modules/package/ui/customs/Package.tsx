...
import { useCustomsTableFilter } from './useFilter';
...
export const CustomsPackageTable = () => {
  ...
  const { isPending: isDownloading, download } = useExcelDownload({
    name: t('packages.status.normalize'),
    params: { ...filter, status: 'in_customs' },
  });
  ...
  const { columnVisibilityModel, handleColumnVisibilityModelChange } =
    usePersistedColumnVisibilityModel('normalizeColumnsVisibility', initialVisibility);
  ...
  return (
    <Box height={700}>
      ...
      <ConfirmDialog
        ...
        action={
          <>
            <Button onClick={archiveDialog.onFalse} variant="contained" color="error">
              {t('actions.not')}
            </Button>
            <LoadingButton
              onClick={onArchive}
      ...
      <ConfirmDialog
        ...
        content={
          <Trans t={t} i18nKey="packages.actions.backToUzb">
