...
export class APIFeatures {
  ...
  filter() {
    ...
    const excluded_fields = ['page', 'sort', 'limit', 'search', 'search_fields']
    ...
    if (this.query_string.search) {
      const search_fields = this.query_string.search_fields?.split(',') || [
        'name',
        'description',
      ]
      const search_conditions = search_fields.map((field) => ({
        [field]: { $regex: this.query_string.search, $options: 'i' },
      }))

      if (Object.keys(sanitized_query_obj).length > 0) {
        this.mongoose_query = this.mongoose_query.find({
          $and: [sanitized_query_obj, { $or: search_conditions }],
        })
      } else {
        this.mongoose_query = this.mongoose_query.find({
